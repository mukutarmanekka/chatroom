...
function appendMessage(data, isSelf = false) {
    ...
    if (data.type === 'system') {
        ...
        if (data.video) {
            contentHtml += `<video controls src="${data.video}"></video>`;
        }
...
socket.on('bot_status', (data) => {
    if (data.isActive) {
        ...
        if (!amIActivator) {
            ...
            audioBtnLabel.classList.add('disabled');
            videoBtnLabel.classList.add('disabled');
            ...
            audioBtnLabel.classList.remove('disabled');
            videoBtnLabel.classList.remove('disabled');
...
audioInput.addEventListener('change', (e) => {
    ...
    if (file) {
        const reader = new FileReader();
        reader.onload = (ev) => {
            selectedAudioDataUrl = ev.target.result;
            audioBtnLabel.style.background = '#dff0d8';
        };
        reader.readAsDataURL(file);
    } else {
        selectedAudioDataUrl = null;
        audioBtnLabel.style.background = '#f1f1f1';
    }
});
...
videoInput.addEventListener('change', (e) => {
    ...
    if (file) {
        const reader = new FileReader();
        reader.onload = (ev) => {
            selectedVideoDataUrl = ev.target.result;
            videoBtnLabel.style.background = '#dff0d8';
        };
        reader.readAsDataURL(file);
    } else {
        selectedVideoDataUrl = null;
        videoBtnLabel.style.background = '#f1f1f1';
    }
});
...
form.addEventListener('submit', (e) => {
    ...
    if (selectedImageDataUrl || selectedAudioDataUrl || selectedVideoDataUrl) {
        socket.emit('chat_message', { 
            text: text, 
            username: username, 
            image: selectedImageDataUrl,
            audio: selectedAudioDataUrl,
            video: selectedVideoDataUrl
        });
